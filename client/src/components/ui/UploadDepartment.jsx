...
export default function UploadDepartment() {
  const [open, setOpen] = React.useState(false);
  const [departments, setDepartments] = React.useState([]);

  const { UploadDepartmentData, setUploadDepartmentData } =
  ...
  const handleSelect = (currentValue) => {
    setUploadDepartmentData(currentValue);
      ...
      <PopoverTrigger asChild>
        ...
        >
          {UploadDepartmentData
            ? departments.find(
                (department) => department.value === UploadDepartmentData
              )?.label
            : "Select Department"}
              ...
              {departments.map((department) => (
                ...
                >
                  ...
                  <CheckIcon
                    className={cn(
                      "ml-auto h-4 w-4",
                      UploadDepartmentData === department.value
                        ? "opacity-100"
                        : "opacity-0"
                    )}
                  />
